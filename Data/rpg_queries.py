...
#DB_FILEPATH = '/Users/keinobaird/Desktop/Data/rpg_db.sqlite3'
...
#COUNT(name)FROM charactercreator_character;'

result = curs.execute(query_1).fetchall()
print(result)
...
query_ave_weapons = """SELECT AVG(weapon_count)FROM
(SELECT cci.character_id,ai.item_id,count(aw.item_ptr_id)
AS weapon_count FROM charactercreator_character_inventory AS cci 
LEFT JOIN armory_item AS ai ON cci.item_id=ai.item_id 
LEFT JOIN armory_weapon AS aw ON aw.item_ptr_id=ai.item_id 
